...
def run_osascript(lines: list[str], *, check: bool = True) -> str:
    ...
    try:
        ...
        raise SystemExit("osascript not found") from exc
    if check:
        return proc.stdout.strip()
    return (proc.stdout or proc.stderr or "").strip()


def macos_frontmost_app_name() -> str | None:
...
def macos_activate_app(app_name: str) -> None:
    safe_name = applescript_quote(app_name)
    subprocess.run(["open", "-a", app_name], check=False, capture_output=True, text=True)
    run_osascript([f'tell application "{safe_name}" to activate'], check=False)


def macos_minimize_front_window(app_name: str) -> None:
    safe_name = applescript_quote(app_name)
    run_osascript(
        [
...
def should_retry_with_restore(args: argparse.Namespace, exc: SystemExit) -> bool:
    if not args.restore_minimized or not args.app:
        return False
    message = str(exc)
    return "no matching macOS window found" in message
...
def main() -> None:
    ...
    parser.add_argument(
        "--app",
        help="macOS only: capture matching on-screen windows for this app name without changing focus",
    )
    parser.add_argument(
        "--window-name",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "macOS only: with --app, briefly activate the app if it has no on-screen window, "
            "then capture it, minimize it again, and restore the previous frontmost app"
        ),
    )
    parser.add_argument(
        "--interactive",
    ...
    if args.restore_minimized and not args.app:
        raise SystemExit("--restore-minimized requires --app")
    if args.restore_minimized and args.list_windows:
        raise SystemExit("--restore-minimized does not support --list-windows")
    if args.restore_minimized and args.active_window:
        raise SystemExit("--restore-minimized does not support --active-window")
    ...
    restored_app = False
    previous_frontmost_app: str | None = None
    ...
    if system != "Darwin" and args.restore_minimized:
        raise SystemExit("--restore-minimized is supported on macOS only")
        ...
        if test_mode:
            ...
            if args.window_id is not None:
                ...
                try:
                    ...
                    if not should_retry_with_restore(args, exc):
                        raise
                    previous_frontmost_app = macos_frontmost_app_name()
                    macos_activate_app(args.app)
                    time.sleep(0.8)
                    window_ids = resolve_macos_windows(args)
                    restored_app = True
            elif args.region is None and not args.interactive:
    ...
    try:
        ...
        if restored_app and args.app:
            macos_minimize_front_window(args.app)
            if previous_frontmost_app and previous_frontmost_app != args.app:
                macos_activate_app(previous_frontmost_app)
