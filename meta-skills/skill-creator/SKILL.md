...
- **Global skills** live in `~/.agents/skills/<skill-name>`.
- **Project-local skills** live in `<project-root>/.agents/skills/<skill-name>`.
...
- Description optimization (`run_loop.py` / `run_eval.py`) should work in a headless worker host as long as the selected backend CLI is available, but please save it until you've fully finished making the skill and the user agrees it's in good shape.
- **Updating an existing skill**: The user might be asking you to update an existing skill, not create a new one. Follow the update guidance in the chat-only host section above.
