...
## v4.4 failure-class eval loop

- Added failure-class extraction to the core output contract so transient bugs and one-off user corrections become reusable behavior evals instead of brittle incident replays.
- Added an evidence-to-eval loop that links run evidence, target skill decision points, patches, validation cases, and old/new comparison plans.
- Added project-based skill guidance: Skill Optimizer owns the optimization loop and rubric, while target skills retain domain-specific source-of-truth order, approval gates, must-not behavior, and validation cases.
- Added a compact five-point behavior-habit rubric for cases where deterministic reproduction is weak.
- Expanded evaluation docs with failure-class cases, behavior-habit evals, and separate reporting for synthetic scores, human rubric scores, and live outcome metrics.
